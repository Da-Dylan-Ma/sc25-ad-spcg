...
# T5: CPU Algorithm Analysis
# This script prepares all required data and runs the CPU algorithm analysis

echo "================================================================================"
echo "T5: CPU Algorithm Analysis"
echo "================================================================================"
...
echo -e "\n1. Computing CPU speedups from raw results..."
cd "$COLLECT_DATA_DIR"
python3 compute_cpu_speedups.py
if [[ $? -ne 0 ]]; then
    echo "Failed to compute CPU speedups"
    exit 1
fi
echo "CPU speedups computed successfully"
...
echo -e "\n4. Computing approximated condition numbers (if not already done)..."
cd "$COLLECT_DATA_DIR"
if [[ ! -f "$LOGS_DIR/approximated_condition_number_inf.csv" ]]; then
    ...
    echo "Approximated condition numbers already exist"
fi

# Check if all required output files were created
output_files=(
    "$LOGS_DIR/cpu_speedups.csv"
...
echo -e "\n5. Running CPU algorithm analysis..."
cd "$ALGORITHM_DIR"
python3 alg_cpu.py
if [[ $? -ne 0 ]]; then
    echo "Failed to run CPU algorithm analysis"
    exit 1
fi
echo "CPU algorithm analysis completed successfully"

echo -e "\n================================================================================"
echo "T5 CPU Analysis Complete!"
echo "================================================================================"
echo "Output files created:"
echo "  - $LOGS_DIR/cpu_speedups.csv"
echo "  - $LOGS_DIR/matrix_application.csv"
echo "  - $LOGS_DIR/wavefronts.csv"
echo "  - $LOGS_DIR/approximated_condition_number_inf.csv"
echo "  - $ALGORITHM_DIR/best_oracle_selection_gpu_pi_from_prediction_cpu.csv"
echo "================================================================================"
