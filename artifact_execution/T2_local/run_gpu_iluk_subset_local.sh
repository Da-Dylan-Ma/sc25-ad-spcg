...
# GPU ILUK Subset Local Execution Script
# This script is equivalent to the cluster version but runs locally

echo "Starting GPU ILUK subset compilation and local execution..."
...
echo "=========================================="
echo "GPU ILUK subset compilations completed successfully!"
echo "=========================================="
...
echo "=========================================="
echo "Running scipy factorization subset jobs locally..."
echo "=========================================="
...
echo "Running subset factorization for all matrices..."

for matrix in "${matrices[@]}"; do
    echo "=========================================="
    echo "Processing subset factorization for: ${matrix}"
    echo "=========================================="
    ...
    if [ ! -f "${matrix_file}" ]; then
        ...
        echo "Skipping ${matrix} subset factorization..."
        echo "-----------------------------"
        continue
    fi
    
    echo "Running subset factorization for: ${matrix_file}"
    
    # Run factorization
    python3 iluk_factorize.py "${matrix_file}"
    
    if [ $? -eq 0 ]; then
        echo "Subset factorization completed successfully for ${matrix}"
    else
        echo "Subset factorization failed for ${matrix}"
    fi
    
    echo "Completed subset factorization for ${matrix}"
    echo "-----------------------------"
done

echo "=========================================="
echo "Subset factorization completed successfully!"
echo "=========================================="
...
echo "=========================================="
echo "Running GPU ILUK subset jobs locally..."
echo "=========================================="
...
GPU_RESULTS_DIR="./subset_results"
...
for matrix in "${matrices[@]}"; do
    ((gpu_counter++))
    echo "=========================================="
    echo "Processing GPU subset job ${gpu_counter}/${total_gpu_jobs}: ${matrix}"
    echo "=========================================="
    ...
    if [ ! -f "${matrix_file}" ]; then
        echo "Matrix file not found: ${matrix_file}"
        echo "Skipping ${matrix} GPU subset job..."
        echo "-----------------------------"
        continue
    fi
    ...
    for factor_dir in ${factors_dir}/${matrix}_*; do
        if [ -d "$factor_dir" ]; then
            ...
            if [ -f "$l_factor" ] && [ -f "$u_factor" ]; then
                ...
                output_file="${GPU_RESULTS_DIR}/iluk_subset_${matrix}_${factor_dir_name}.out"
                ...
                if [ $? -eq 0 ]; then
                    echo "GPU ILUK subset job completed successfully for ${matrix} with ${factor_dir_name}"
                    matrix_processed=true
                else
                    echo "GPU ILUK subset job failed for ${matrix} with ${factor_dir_name}"
                fi
            else
    ...
    echo "Completed GPU subset job for ${matrix} (${gpu_counter}/${total_gpu_jobs})"
    echo "-----------------------------"
done

echo "=========================================="
echo "GPU ILUK subset local execution completed successfully!"
echo "=========================================="
echo "Summary:"
echo "  - Executable: ${BASE_DIR}/build/conjugateGradientPrecond"
echo "  - GPU subset jobs completed: ${gpu_counter}/${total_gpu_jobs}"
